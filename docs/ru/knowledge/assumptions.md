...
| A-003 | MQTT может быть начальным локальным event transport, не раскрывая MQTT topics в доменной модели OSIP. | Ревью контрактов и адаптеров; тест имитации альтернативного producer. | Усилить границу адаптера или пересмотреть решение о backbone. | До первой production-like интеграции |
...
| A-005 | Облачные capability могут быть необязательны для первых эксплуатационных use cases. | Выполнить сценарии потери WAN для всех Local First-функций. | Переклассифицировать функции, добавить локальные компоненты либо сократить охват. | Review архитектурной базы |
| A-006 | Платформа может начать с ограниченного явного каталога capability, а не стремиться поддержать все устройства. | Утвердить начальную матрицу capability и отслеживать неподдерживаемые запросы. | Расширять каталог через версионированные контракты и валидацию reference design. | Каждое планирование релиза |
